...
const Dashboard = () => {
  ...
  const unpaidInvoice = invoices?.filter(
    (invoice) => invoice.status === "Non payé"
  );
  const paid = invoices?.filter((invoice) => invoice.status === "Payé");
  const partial = invoices?.filter((invoice) => invoice.status === "Partielle");
    ...
    return (
      ...
      >
        ...
        <p style={{ padding: "40px", color: "gray" }}>
          Rien à afficher. Cliquez sur l'icône plus pour commencer à créer
        </p>
      </div>
    );
  }
        ...
        <ul className={styles.autoGrid}>
          ...
          >
            <div>
              ...
              <h2 style={{ color: "white" }}>Paiement reçu</h2>
            </div>
            <div>
              <Check />
            </div>
          ...
          >
            <div>
              <p>{paid.length}</p>
              <h2 style={{ color: "white" }}>Factures payées</h2>
            </div>
            <div>
              <Check />
            </div>
              ...
              <tbody>
                {paymentHistory.length !== 0 && (
                  <tr>
                    ...
                    <th style={{ padding: "15px" }}>Payé par</th>
                    <th style={{ padding: "15px" }}>Date de paiement</th>
                    <th style={{ padding: "15px" }}>Le montant payé</th>
                    <th style={{ padding: "15px" }}>Mode de paiement</th>
